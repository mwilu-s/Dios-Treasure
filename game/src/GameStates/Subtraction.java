 ...
 * @author User
 */
public class Subtraction extends State implements StateMethods {
   ...
   private GameHelpOverlay gHelp;
    ...
    public void checkPotionTouched(Rectangle2D.Float collision){
        om.checkObjectTouched(collision, GameState.SUBTRACTION);
    }
    ...
    public void update() {//updates all the overlays and objects as well as the player and enemies and all their animations and states
        ...
        if(paused){
            ...
            player.update(GameState.SUBTRACTION);
        }else {
            lm.update();
   ...
   //checks how far the player is from the border walls of the leve
    ...
    public void mouseReleased(MouseEvent e) {
        if(!gameOver){
        if(isIn(player.getHelpBtn(),e)){
            player.mouseReleased(e, GameState.SUBTRACTION);
        }
        if(paused){
            po.mouseReleased(e, GameState.SUBTRACTION);
        }
        }else{
            gOver.mouseReleased(e,GameState.SUBTRACTION);
        }
    }
