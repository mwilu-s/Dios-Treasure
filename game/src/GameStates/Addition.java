 ...
 * @author User
 */
public class Addition extends State implements StateMethods{
   ...
   private int rndNum = (int) (Math.random() *2) +1;
   ...
   private GameHelpOverlay gHelp;
    ...
    /**
     *This initializes all the classes necessary for the game to be played
     * @param game : the game object
     */
    public Addition(Game game){
        super(game);
        initClasses();
    ...
    public void checkPotionTouched(Rectangle2D.Float collision){
        om.checkObjectTouched(collision, GameState.ADDITION);
    }
    ...
    public void update() {//updates all the overlays and objects as well as the player and enemies and all their animations and states
        ...
        if(paused){
            ...
            player.update(GameState.ADDITION);
        }else {
            lm.update();
    ...
    public void mouseReleased(MouseEvent e) {
        if(!gameOver){
        if(isIn(player.getHelpBtn(),e)){
            player.mouseReleased(e, GameState.ADDITION);
        }
        if(paused){
            po.mouseReleased(e, GameState.ADDITION);
        }
        }else{
            gOver.mouseReleased(e, GameState.ADDITION);
        }
    }
